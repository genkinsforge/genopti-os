...
def get_update_status():
    ...
    try:
        ...
        logging.error(f"Error getting update status: {e}")
        return jsonify({
            'success': False,
...
def install_update():
    ...
    try:
        ...
        return jsonify({
            'success': False,
            'error': f'Failed to install update: {e}'
        }), 500
...
def get_update_history():
    """Get update history."""
    try:
        if not AUTO_UPDATE_AVAILABLE or not version_manager:
            return jsonify({
                'success': False,
                'error': 'Auto-update system not available'
            }), 503
...
def create_backup():
    ...
    try:
        if not AUTO_UPDATE_AVAILABLE or not backup_manager:
            return jsonify({
                'success': False,
        ...
        logging.info(f"Creating backup: {backup_name or 'auto-generated'}")
        
        backup_path = backup_manager.create_backup(backup_name, include_logs)
...
def get_system_info():
    ...
    try:
        from version_manager import get_system_info
        
        current_version = version_manager.get_current_version() if version_manager else APP_NAME_VERSION
        ...
        response = {
            ...
            'data': {
                ...
                'features': {
                    ...
                    'auto_update': AUTO_UPDATE_AVAILABLE
                }
            }
        }
